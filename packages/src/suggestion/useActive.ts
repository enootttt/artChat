...
export default function useActive(
  ...
  const offsetRow = (offset: number) => {
    ...
    const currentRowIndex = currentItems.findIndex((item) => item.value === activePaths.value[currentColIndex - 1]);
    const itemCount = currentItems.length;

    const nextItem = currentItems[(currentRowIndex + offset + itemCount) % itemCount];
    activePaths.value = [...activePaths.value.slice(0, currentColIndex - 1), nextItem.value as string];
  };
